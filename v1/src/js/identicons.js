...
${content}
  ...
  static async _generatePart(part, index) {
    ...
    if (!$part)
      return '' // Required for catalog.html and as a safeguard
  ...
  static async _getAssets() {
    return this._assetsPromise || (this._assetsPromise = new Promise(async (resolve) => {
      let assetsText
      if (typeof IdenticonsAssets !== 'undefined') {
        assetsText = IdenticonsAssets
      } // Check for inlined assets
      else {
        assetsText = await fetch(self.NIMIQ_IDENTICONS_SVG_PATH || Identicons.svgPath)
